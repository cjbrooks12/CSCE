...
DoublyLinkedList::DoublyLinkedList(DoublyLinkedList& dll)
{
  ...
  while (p!=dll.getAfterLast()) {
    DListNode *newNode = new DListNode(p->getElem(), trailer.prev,&trailer);
...
// assignment operator
DoublyLinkedList& DoublyLinkedList::operator=(DoublyLinkedList& dll) //O(n), two loops each O(n) is still O(n)
{
...
// output operator
ostream& operator<<(ostream& out, DoublyLinkedList& dll) { //O(n)
