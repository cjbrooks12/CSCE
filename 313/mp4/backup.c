...
void process_returned_value(string returned) {
    ...
    if(returned.compare(0, returned.size()-2, "data John Doe")) {
        people[0]->stat_buffer->deposit(data);
    }
    else if(returned.compare(0, returned.size()-2, "data Joe Smith")) {
        people[1]->stat_buffer->deposit(data);
    }
    else if(returned.compare(0, returned.size()-2, "data Jane Smith")) {
...
void* event_handler(void* argument) {
    ...
    for(int i = 0; i < num_channels; i++) {
        ...
        maxfd = max(maxfd, channels[i]->write_fd());
    ...
    while(items_read < num_requests*people.size()) {
        FD_ZERO(&readset); 
        ...
        for(int i = 0; i < channels.size(); i++) {
            if(items_written < num_requests*people.size()) {
                ...
                channels[i]->cwrite(request); 
                items_written++;
                //cout << "LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL\n";
            }
        ...
        //cout << returned << " written: " << items_written << " read: " << items_read << "\n";
        if(items_written == items_read == num_requests*people.size()) {
            // cout << "HHHHHHHHHHHHHHHHHHHHHHHHHHHHHH\n";
            break;
        }
    } 
...
void run_program() {
    ...
    pthread_join(JohnDoe->request_t, NULL);
    pthread_join(JoeSmith->request_t, NULL);
    pthread_join(JaneSmith->request_t, NULL);
    
    pthread_join(JohnDoe->stat_t, NULL);
    pthread_join(JoeSmith->stat_t, NULL);
    pthread_join(JaneSmith->stat_t, NULL);
